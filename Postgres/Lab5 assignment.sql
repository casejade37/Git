...
DROP TABLE Store_location CASCADE;
DROP TABLE Alternate_name CASCADE;
DROP TABLE Product CASCADE;
DROP TABLE Currency CASCADE;
DROP TABLE Shipping_offering CASCADE;
...
SELECT * FROM Sells ORDER BY product_id;
SELECT * FROM Offers;
...
SELECT to_char(price_in_us_dollars *
             ...
             FROM Currency
             WHERE currency_name = 'British Pound'),
              '£999.99') AS price_in_pounds
FROM Product
...
WHERE Product.product_id IN
    (SELECT Product.product_id
     ...
     HAVING COUNT(Sells.store_location_id) = (SELECT COUNT(DISTINCT store_location_id) FROM Sells));
...
FROM (SELECT Product.product_id
     ...
     ON Product.product_id = Sells.product_id
     GROUP BY Product.product_id
     HAVING COUNT(Sells.store_location_id) = (SELECT COUNT(DISTINCT store_location_id) FROM Sells)) Popular_product
JOIN Product ON product.product_id = Popular_product.product_id
JOIN Alternate_name ON product.product_id = Alternate_name.product_id;

SELECT Product.product_name, Alternate_name.name
FROM Product
JOIN Alternate_name ON product.product_id = Alternate_name.product_id
WHERE EXISTS (SELECT Correlated_product.product_id
             FROM Product Correlated_product
             JOIN Sells ON Correlated_product.product_id = Sells.product_id
             WHERE Product.product_id = Correlated_product.product_id
             GROUP BY Correlated_product.product_id
             HAVING COUNT(Sells.store_location_id) = (SELECT COUNT(DISTINCT store_location_id) FROM Sells));
...
SELECT Product.product_name, Alternate_name.name
FROM Product_view
JOIN Product ON Product_view.product_id = Product.product_id
JOIN Alternate_name ON Product_view.product_id = Alternate_name.product_id